	...
	binaryOptions: {
		...
		optionTransactions({ max = Infinity, market = undefined, account = undefined } = {}) {
			return pageResults({
				...
				results.map(
					({ id, timestamp, type, account, currencyKey, side, amount, market, fee, claimedLong, claimedShort }) => ({
						hash: id.split('-')[0],
						timestamp: Number(timestamp * 1000),
						type,
						account,
						...
						amount: amount ? amount / 1e18 : null,
						market,
						fee: fee ? fee / 1e18 : null,
						claimedLong: claimedLong ? claimedLong / 1e18 : null,
						claimedShort: claimedShort ? claimedShort / 1e18 : null,
					}),
				),
			);
		},
